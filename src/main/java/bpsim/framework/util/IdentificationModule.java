...
public class IdentificationModule {
	private String ROOT_DOMAIN = CommonUtil.getAppRes("Appcommon.ROOT_DOMAIN");
	...
	public IdentificationModule(HttpSession session){
		this._session = session;
		this.sErrorUrl = ROOT_DOMAIN + "join.do?cmd=checkplus_fail";		  // 실패시 이동될 URL
		this.sReturnUrl	= ROOT_DOMAIN+"join.do?cmd=checkplus_success";
		this.iReturnUrl	= ROOT_DOMAIN + "join.do?cmd=ipin_result";
		...
		iReturnUrl	= ROOT_DOMAIN + "join.do?cmd=ipin_result";
		
		// Method 결과값(iRtn)에 따라, 프로세스 진행여부를 파악합니다.
		int iRtn = pClient.fnRequest(ipinCode, ipinPw, sCPRequest, iReturnUrl);
System.out.println("ipin result-url:"+iReturnUrl);		
